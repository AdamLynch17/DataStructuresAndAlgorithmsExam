...
public:
    ...
    void set(int p, T item); // set node ps item to item  
...
bool List<T>::remove(int p1, int p2)
{
    ...
    for (int p = p1; p <= p2; p++)
        remove(p1);
    /* implementation by merging the code for remove(int p),
remove_front(), and remove_end() */
/*
...
while (p1 <= p2 && pCurrentNode != NULL) {
         ...
         // pCurrentNode points to the last node
                  ...
                  pCurrentNode->pPrevNode->pNextNode = pCurrentNode-
>pNextNode;
                           pCurrentNode->pNextNode->pPrevNode = pCurrentNode-
>pPrevNode;
                           delete pCurrentNode;
                  }
                  listSize--;
